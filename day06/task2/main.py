...
import copy
from utils import search_matrix, pad_matrix
import sys
import pathlib
    ...
    while True:
        if not last_three_turns[0] and last_three_turns[1] and not last_three_turns[2]:
            ...
            if reversed:
                return True
            reversed = True
        next_tile = guard_map[guard_pos[0] + current_direction[0]
                              ][guard_pos[1] + current_direction[1]]
        if next_tile[0] == '#':
            ...
            last_three_turns = [
                False, last_three_turns[0], last_three_turns[1]]
        guard_map[guard_pos[0]][guard_pos[1]][0] = 'X'
        guard_map[guard_pos[0]][guard_pos[1]
                                ][1] = directions.index(current_direction)
